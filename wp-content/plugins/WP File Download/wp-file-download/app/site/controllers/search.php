...
class WpfdControllerSearch extends Controller
{
    ...
    public function getTagByCatId()
    {
        ...
        if (!is_numeric($catId)) {
            ...
            if ($catType === 'onedrive' && has_filter('wpfdAddonSearchOneDrive', 'wpfdAddonSearchOneDrive')) {
                ...
                $files = apply_filters('wpfdAddonGetListOneDriveFile', $id_category, 'ordering', 'asc', $catSlug, $token);
                ...
                if (!empty($odFileInfos)) {
                    foreach ($odFileInfos as $odKey => $odValues) {
                        foreach ($odValues as $odId => $odValue) {
                            ...
                            if (isset($odValue['file_tags']) && $odValue['file_tags'] !== '') {
