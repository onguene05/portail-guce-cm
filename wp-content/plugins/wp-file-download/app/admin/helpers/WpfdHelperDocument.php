...
class WpfdHelperDocument
{
    ...
    public function __construct($postid, $filepath)
    {
        ...
        if (defined('WPFD_DOCUMENT_PARSE_DEBUG') && WPFD_DOCUMENT_PARSE_DEBUG) {
            $this->debug = true;
        }
    }
    ...
    private function getPdfContent()
    {
        ...
        $pdfContent  = '';
        // todo: Use https://github.com/spatie/pdf-to-text if exists on server
...
//        $content = strip_tags($content);
     ...
     *
     * @param mixed   $message Log message or variable
     * @param integer $trim    Length of the print message
     * @param string  $wrapper Print with wrapper in log file
     *
     * @return void
     */
    protected function log($message = '', $trim = 0, $wrapper = '')
    {
        if (!$this->debug) {
            return;
        }
        ...
        // phpcs:disable WordPress.PHP.DevelopmentFunctions.error_log_error_log,WordPress.PHP.DevelopmentFunctions.error_log_var_export -- Debug purposed
        if (!empty($wrapper)) {
            error_log('================= BEGIN ' . $wrapper . '=================');
        }
        if (is_string($message)) {
            if (0 !== $trim) {
                $message = substr($message, 0, $trim) . '...';
            }
            error_log($message);
        } else {
