...
class WpfdModelFile extends Model
{
    ...
    public function getFile($id_file)
    {
        ...
        $row['publish'] = get_date_from_gmt($row['post_date_gmt'], get_option('date_format', 'Y-m-d') . ' ' . get_option('time_format', 'H:i:s'));
        // We save in GMT so always convert to current date
        $row['expiration'] = $expirationDate ? get_date_from_gmt($expirationDate, get_option('date_format', 'Y-m-d') . ' ' . get_option('time_format', 'H:i:s')) : '';
        $row['file_password'] = isset($row['post_password']) ? $row['post_password'] : '';
        ...
        if ($viewer_type !== 'no' &&
            (isset($row['ext']) && in_array($row['ext'], $extension_viewer))
            && ($remote_url === false)
        ) {
            $row['viewer_type'] = $viewer_type;
            $row['viewerlink'] = WpfdHelperFile::getMediaViewerUrl($row['ID'], $row['catid'], $row['ext'], $token);
        }
    ...
    public function save($datas)
    {
        ...
        $postDate = date('Y-m-d H:i:s', strtotime($datas['publish']));
        $datas['publish'] = $postDate;

        if ($datas['expiration']) {
            $exPostDate = date('Y-m-d H:i:s', strtotime($datas['expiration']));
            // Always save GMT date
            $datas['expiration'] = get_gmt_from_date($exPostDate, 'Y-m-d H:i:s');
        }

        $postDateGmt = get_gmt_from_date($datas['publish']);
        $futureTime  = strtotime($postDateGmt . ' GMT');
        ...
        $my_post = array(
            ...
            'post_password' => isset($datas['file_password']) ? $datas['file_password'] : ''
        );
        ...
        // Future publish
        $my_post['post_status'] = ( $futureTime > time() ) ? 'future' : $my_post['post_status'];
     ...
     * @param array $additionals Param datas
     *
     * @return void
     */
    public function addVersion($file, $additionals = array())
    {
        $metadata                 = array();
        ...
        if (is_array($additionals) && count($additionals)) {
            $metadata = array_merge($metadata, $additionals);
        }
     ...
     * @return integer
     */
    public function getVersionCount($fileId)
    {
        global $wpdb;
        $results  = $wpdb->get_var($wpdb->prepare(
            'SELECT COUNT(*) FROM ' . $wpdb->postmeta . ' WHERE post_id = %d AND meta_key = %s ORDER BY meta_id DESC',
            (int) $fileId,
            '_wpfd_file_versions'
        ));

        /**
         * Filter allow change automatic version increment starting number
         *
         * @param integer
         */
        return is_null($results) ? apply_filters('wpfd_version_increment_number_start', 0) : (int) $results;
    }
