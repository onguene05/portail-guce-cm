...
class WpfdModelFts extends Model
{
    ...
    {
        $this->log[] = $message;
        // phpcs:disable WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Debug purposed
        if ($this->debug) {
            error_log($message);
        }
        // phpcs:enable
    ...
    protected function splitToWords($str)
    {
        ...
        if (is_array($matches[1])) {
            foreach ($matches[1] as $word) {
                if ($this->isJapanese($word)) {
                    $splitedWords = mb_str_split($word); // phpcs:ignore PHPCompatibility.FunctionUse.NewFunctions.mb_str_splitFound -- Polyfill ready
                    $words = array_merge_recursive($words, $splitedWords);
                } else {
                    $words[] = $word;
                }
            }
        ...
        return apply_filters('wpfd_fts_split_to_words', $words, $str);
    }
    ...
    public function add($docs = array())
    {
        ...
        foreach ($docs as $id => $doc) {
            ...
            foreach ($words as $word) {
                ...
                $this->log('Processing: ' . $word);
                if (($this->isJapanese($word) || $len > 1) && ($len <= $this->max_word_length) && (!isset($this->stops[$lv]))) {
                    if (!isset($word2[$lv])) {
                        $word2[$lv] = 1;
                    } else {
                        $word2[$lv]++;
        ...
        foreach ($wordlist_ch as $d) {
            ...
            foreach ($d as $dd) {
                $this->log('Inserting words list: ' . $dd[0]);
                $z[] = '("' . addslashes($dd[0]) . '")';
            }
