...
class WpfdTheme
{
    ...
    public function showCategory($options, $search = false)
    {
        ...
        if (empty($options['files']) && empty($options['categories']) && !$showUploadForm) {
            if (is_admin()) {
    ...
    public static function showTitleHandlebars($config, $params)
    {
        ...
        if ((int) WpfdBase::loadValue($params, self::$prefix . 'showtitle', 1) === 1) {
            $name     = self::$themeName;
            $template = array(
                'html' => '<h3>' . $selectFileInput . '<a href="%url$s" %data$s class="wpfd_downloadlink" title="%title$s" target="%target$s">%text$s</a></h3>',
                'args' => array(
                    'url'   => '{{linkdownload}}',
    ...
    public static function showDownloadHandlebars($config, $params)
    {
        if ((int) WpfdBase::loadValue($params, self::$prefix . 'showdownload', 1) === 1) {
            ...
            $template = array(
                'html' => '<a class="%class$s" %data$s href="%url$s" style="%style$s" target="%target$s">%text$s%icon$s</a>',
                'args' => array(
    ...
    public static function wpfdShowUploadFormSubCategories($categoryId)
    {
        ...
        foreach ($subCates as $cate) {
            ...
            if (isset($pararms['showuploadform']) && (int) $pararms['showuploadform'] === 1) {
                $result = true;
                return $result;
            }
        }
