 ...
 */
    ...
    $businoz_video_url = function_exists( 'get_field' ) ? get_field( 'fromate_style' ) : NULL;
...
?>
    ...
    <article id="post-<?php the_ID();?>" <?php post_class( 'bd-postbox__item  mb-40 format-video' );?>>
        <?php if ( has_post_thumbnail() ): ?>    
        <div class="bd-postbox__thumb w-img position-relative">
            <a href="<?php the_permalink();?>">
                <?php the_post_thumbnail( 'full', ['class' => 'img-responsive'] );?>
            </a>

            <?php if(!empty($businoz_video_url)) : ?>
            <a href="<?php print esc_url( $businoz_video_url );?>" class="play-icon play-icon-ab popup-video"><i class="fas fa-play"></i></a>
            <?php endif; ?>
        </div>
        <?php endif; ?>
